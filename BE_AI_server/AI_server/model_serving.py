...
if __name__ == "__main__":
    # A public tunnel is only a local-development convenience. It must never be
    # created as an import side effect or be enabled in a production container.
    if os.getenv("ENABLE_NGROK", "false").lower() == "true":
        auth_token = os.getenv("NGROK_AUTH_TOKEN")
        if not auth_token:
            raise RuntimeError("NGROK_AUTH_TOKEN is required when ENABLE_NGROK=true")
